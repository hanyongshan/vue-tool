<template>
  <div class="ms-date-range">
    <el-date-picker
      ref="dateStartRef"
      class="ms-date-picker start"
      :style="dateRangePickerStyle"
      popper-class="ms-date-picker-popper"
      v-model="dateStart"
      @change="dateChange"
      @focus="getPickerStart"
      :type="type"
      :format="format"
...
export default {
    ...
    computed: {
        ...
        pickerOptionsStart() {
            const disabledDateObj = this.limitDateNow
                ? this.$msValidate.noBigDateNoOverToday(this.$data, 'dateEnd')
                : this.$msValidate.noBigDate(this.$data, 'dateEnd');
            if (this.type === 'date') return disabledDateObj;
            return { ...disabledDateObj, ...this.setShortcuts() };
        },
        pickerOptionsEnd() {
            const disabledDateObj = this.limitDateNow
                ? this.$msValidate.noSmallDateNoOverToday(this.$data, 'dateStart', true)
                : this.$msValidate.noSmallDate(this.$data, 'dateStart', true);
            if (this.type === 'date') return disabledDateObj;
            return { ...disabledDateObj, ...this.setShortcuts() };
        },
        format() {
    ...
    methods: {
        ...
        getDateStart() {
            return this.$msSystem.formatDate(
                this.dateStart,
                this.type === 'datetime'
                    ? this.ifSeconds
                        ? 'YYYY-MM-DD HH:mm:ss'
